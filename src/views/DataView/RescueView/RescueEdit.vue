...
// 儲存原始檔案（用於上傳）
let imageFile = null
let rescueIdNumber = null // 儲存數字 ID（不含 #）
...
const loadRescueData = async () => {
  ...
  try {
    const response = await backHomeApi.get(`./savedcases/rescue_get.php?id=${id}`)
    const data = response.data

    // 後端回傳的資料 map 到 formData
    rescueIdNumber = data.id // 儲存數字 ID
    formData.id = formatRescueId(data.id)
    formData.name = data.name
    formData.species = data.species
    formData.location = data.location
    formData.status = data.status
    formData.description = data.description
    formData.uploadDate = data.uploadDate

    // 圖片預覽
    if (data.imageSrc) {
      formData.imageUrl = fileUrl + data.imageSrc
      formData.imageName = data.imageSrc.split('/').pop() // 取得檔名
    }
  } catch (error) {
...
const handleSubmit = async () => {
  ...
  submitData.append('id', rescueIdNumber)
  ...
  try {
    const response = await backHomeApi.post('./savedcases/rescue_update.php', submitData, {
      ...
      <el-form :model="formData" label-width="120px" label-position="left" class="customForm">
        <el-row :gutter="40">
          ...
          <el-col :span="10" :offset="4">
            <el-form-item label="上傳日期" label-width="140px">
              <el-input v-model="formData.uploadDate" disabled class="readOnlyInput" />
            </el-form-item>
          </el-col>
        </el-row>
